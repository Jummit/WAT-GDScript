...
static func split(tests: Array, controller, threads: int = 1) -> Array:
	tests.sort_custom(YieldTimeSorter, "sort_ascending")
	threads = calibrate_threads(tests.size(), threads)
	return _testthreads(_distribute(_testpools(threads), tests, threads), controller)
